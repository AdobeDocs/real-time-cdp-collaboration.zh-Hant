...
若要設定Real-Time CDP Collaboration的存取控制，您&#x200B;**必須**&#x200B;擁有系統或產品管理員許可權。 系統管理員沒有限制，並在上線流程中布建。 同時，產品管理員可以為他們被指派的所有產品提供管理功能。 產品管理員必須由系統管理員授予產品和管理存取權。
...
在授予Real-Time CDP Collaboration存取權之前，您需要先存取多項產品，具體取決於您的[使用案例](#use-cases)。 存取控制指南可在您進行時透過多個使用者介面運作，每個介面都可在存取設定過程中提供特定用途。 請參考下表，深入瞭解每項產品的用途。
...
| 擁有&#x200B;**Experience Platform UI存取權的現有Experience Platform系統管理員**&#x200B;的超級使用者。 | 系統管理員。 | [設定Real-Time CDP Collaboration存取權](./manage-user-access.md#RTCDP-collab-access) |
| 現有Experience Platform系統管理員&#x200B;**的超級使用者，無** Experience Platform UI存取權。 | 系統管理員。 | [設定產品管理員存取權](./manage-user-access.md#admin-access) |
| 產品管理員許可權和新產品管理員的Real-Time CDP Collaboration存取權。 | 系統管理員。 | [設定產品管理員存取權](./manage-user-access.md#admin-access) |
| 現有Experience Platform產品管理員&#x200B;**的Real-Time CDP Collaboration存取權，具有** Experience Platform UI存取權。 | 系統或產品管理員。 | [設定Real-Time CDP Collaboration存取權](./manage-user-access.md#RTCDP-collab-access) |
| 現有Experience Platform產品管理員&#x200B;**的Real-Time CDP Collaboration存取權(無** Experience Platform UI存取權)。 | 系統或產品管理員。 | [設定使用者存取權](./manage-user-access.md#user-access) |
| 新一般使用者的Real-Time CDP Collaboration存取權。 | 系統或產品管理員。 | [設定使用者存取權](./manage-user-access.md#user-access) |
...
取得Real-Time CDP Collaboration的存取權後，您的特定功能可能需要一些額外的Experience Platform許可權。

### 對象匯入 {#audience-importation}

開始與共同作業人員共用受眾之前，您需要將受眾匯入Real-Time CDP Collaboration。 目前，匯入對象唯一支援的資料連線為Experience Platform。 若要開始，管理對象上線的使用者必須獲派包含下列&#x200B;**設定檔管理**&#x200B;資源許可權的角色：

| 權限 | 說明 |
| ---- | ---- |
| 檢視區段 | 允許使用者檢視沙箱中可用的受眾清單。 |
| 檢視設定檔 | 允許使用者檢視可用於對應至共同作業欄位的欄位。 |
...
>[!NOTE]
>
>使用者匯入後，便可在Real-Time CDP Collaboration中使用對象，而無需上述任何許可權。

## 後續步驟

決定好從何處開始之後，請依照使用案例的連結，開始設定存取權。 如果您想瞭解如何在這些使用案例之外以管理員身分設定存取Real-Time CDP Collaboration的許可權，請參閱[管理使用者存取權](manage-user-access.md)指南。 若要瞭解角色及其在設定Real-Time CDP Collaboration各種元件存取權時所扮演的角色，請參閱[管理角色](manage-roles.md)指南。
