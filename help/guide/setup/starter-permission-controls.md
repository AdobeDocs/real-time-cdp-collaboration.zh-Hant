---
title: 設定Collaboration入門人員的許可權控制項
description: 瞭解如何使用Adobe Experience Cloud中的許可權設定Adobe Real-Time CDP Collaboration Starter的許可權。
audience: users invited to Real-Time CDP Collaboration Starter
badgelimitedavailability: label="有限可用性" type="Informative" url="https://helpx.adobe.com/tw/legal/product-descriptions/real-time-customer-data-platform-collaboration.html newtab=true"
hide: true
exl-id: 4e50b6cc-58f7-4a0c-8b6d-f5aa4f092e9f
source-git-commit: 61545ee26359d06191a55d0d96103dc41852cc3a
workflow-type: tm+mt
source-wordcount: '48'
ht-degree: 31%

---

# 設定Collaboration入門人員的許可權控制項
