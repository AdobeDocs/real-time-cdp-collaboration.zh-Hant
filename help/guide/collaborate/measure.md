...
此檢視提供您選擇以表格格式測量的轉換事件的詳細劃分。 結果是在&#x200B;**促銷活動識別碼**&#x200B;層級上計算。 此表格包含：

* **轉換事件**：您正在追蹤的每個轉換事件名稱。
* **轉換計數**：行銷活動ID中每個事件發生的轉換總數。
* **轉換值**：促銷活動ID歸因於每個轉換事件的值。
...
### 依位置轉換 {#conversions-by-placement}

此表格提供針對歸因報表選取之轉換事件的&#x200B;**位置ID層級劃分**。 單一行銷活動ID可包含多個位置ID，因此使用此檢視來檢視行銷活動中的轉換劃分方式。 針對每個選取的轉換事件，表格會顯示歸因於每個位置的轉換以及對應的轉換值。 此表格包含：

* **刊登版位名稱或ID**：創意執行所在刊登版位的識別碼。
* **轉換計數**：針對建立報表時選取的每個轉換事件，歸因於該位置的轉換計數。 位置可顯示最多三個轉換事件，符合每個報告允許的轉換事件數上限。
* **轉換值**：歸因於該特定位置的每個轉換事件的值。

![依位置轉換。](/help/assets/collaborate/measure/conversions-by-placement.png)
